...
const createImg = function(link, previewImgLink, description, index) {
...
const imageSetSrc = event => {
  imageModalRef.setAttribute('src', `${getOriginalUrl(event)}`);
  imageModalRef.setAttribute('alt', `${getDescription(event)}`);
};
...
const closeModal = () => {
  ...
  overlayRef.removeEventListener('click', closeModalBackdrop);
};
...
const keyRightLeftClick = event => {
  ...
  imageModalRef.setAttribute('src', `${items[imgIndex].original}`);
  imageModalRef.setAttribute('alt', `${items[imgIndex].description}`);
};
